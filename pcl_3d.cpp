...
std::vector<ClusterInfo> PCL_3D::findBoundingBox(const std::string& filePathBox,
    ...
    Eigen::Vector3f absPrevLocation = prevLocation + referencePoint;

    // Determine the bounding box around the previous location in the point cloud's original coordinates
    pcl::PointXYZ minPt, maxPt;

    //Half the dimensions
    Eigen::Vector3f halfDimensions = dimensions*1.6f;
    ...
    if(prevLocation.isZero()) {
        ...
        if(dimensions.isZero()){
            ...
            minPt = {absPrevLocation.x() - 300, absPrevLocation.y() - 300, absPrevLocation.z() - 2500};
            maxPt = {absPrevLocation.x() + 300, absPrevLocation.y() + 300, absPrevLocation.z()};
        }else{
            minPt = {absPrevLocation.x() - halfDimensions.x(), absPrevLocation.y() - halfDimensions.y(), absPrevLocation.z() - 2500};
            maxPt = {absPrevLocation.x() + halfDimensions.x(), absPrevLocation.y() + halfDimensions.y(), absPrevLocation.z()};
        }
                                                         ...
                                                         const Eigen::Vector3f& prevLocation)
{
    ...
    if(prevLocation.isZero()){
        ...
        minPt = {prevLocation.x() - 200, prevLocation.y() - 200, prevLocation.z() -2500};
        maxPt = {prevLocation.x() + 200, prevLocation.y() + 200, prevLocation.z() + 2500};
    }
